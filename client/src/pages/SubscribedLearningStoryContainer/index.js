...
const SubscribedLearningStoryContainer = ({ handleViewStory, latestOnly }) => {
  ...
  const handleUnsubscribeStory = async id => {
    try {
      setContainerStatus("processing");
      const selectedLearningStory = subscribedLearningStories.filter(
        ls => ls._id === id
      );
      await API.unSubscribeLearningStory(id);
      const filteredLearningStories = subscribedLearningStories.filter(
        ls => ls._id !== id
      );
      ...
      setMessage({
        message: `You have successfully unsubscribed from 
            ${selectedLearningStory[0].title}. We will miss you. See you soon.`,
        severity: "success"
      });
      // wait for 3 seconds
  ...
  if (!subscribedLearningStories || containerStatus === "processing") {
      ...
      <Container className={classes.root}>
        <SubscribedLearningStoryListItem
          subscribedLearningStories={
            latestOnly
              ? subscribedLearningStories.slice(0, 1)
              : subscribedLearningStories
          }
    ...
    subscribedLearningStories &&
    subscribedLearningStories.length === 0
  ) {
    return (
      <Container component="main" maxWidth="md">
        <MessageAlert
