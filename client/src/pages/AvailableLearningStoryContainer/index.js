...
const AvailableLearningStoryContainer = ({ handleViewStory, latestOnly }) => {
  ...
  const handleSubscribeStory = async id => {
    try {
      setContainerStatus("processing");
      const selectedLearningStory = availableLearningStories.filter(
        ls => ls._id === id
      );
      await API.subscribeLearningStory(id);
      const filteredLearningStories = availableLearningStories.filter(
        ls => ls._id !== id
      );
  ...
  if (!availableLearningStories || containerStatus === "processing") {
      ...
      <Container className={classes.root}>
        <AvailableLearningStoryListItem
          availableLearningStories={
            latestOnly
              ? availableLearningStories.slice(0, 1)
              : availableLearningStories
          }
    ...
    availableLearningStories &&
    availableLearningStories.length === 0
  ) {
    return (
      <Container component="main" maxWidth="md">
        <MessageAlert
...
AvailableLearningStoryContainer.propTypes = {
